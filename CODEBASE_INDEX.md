...
> Generated: 2026-04-29 · Refreshed: 2026-05-09, 2026-05-17, 2026-05-18 · Run the codebase-onboarding skill again to refresh.
...
app/page.tsx (homepage) ── components/sections/* (Nav, Hero, …, WaitlistCTA, Footer)
   Nav CTA + FAQ "Talk to us" ── components/ui/EnquiryModal ── lib/useEnquirySubmit
   WaitlistCTA inline form    ── components/forms/EnquiryFormFields (theme="dark")
app/onboard/page.tsx    ── useReducer(OnboardState) + phase machine  (unlinked from public nav, noindex)
...
app/upload/page.tsx  ── magic-link sign-in → upload logo + photos to private storage
       ...
       │           │           │
       │ submitForm(NEXT_PUBLIC_ENQUIRY_SUBMIT_URL,  {name,email,businessType,suburb,servicesInterested,message,source})
...
──────────────────
Postgres:
  enquiries           (RLS: admin_read_all_enquiries + admin_update_enquiries)
...
Edge Functions (Deno):
  enquiry-submit       — public, honeypot + email regex, inserts enquiry + admin/client emails
...
| `app/onboard/page.tsx` | 10-step intake wizard. Owns reducer, phase, step, validate, submit. **Unlinked from public nav** — `noindex`. Admin shares URL to qualified leads. |
| `app/admin/page.tsx` | Internal dashboard (auth: password, gated to `ADMIN_EMAIL`). **Tabs: Enquiries \| Submissions** |
...
| `components/forms/EnquiryFormFields.tsx` | Shared enquiry fields (name/email/biz type/suburb/services multi-select/honeypot). Theme prop `'dark' \| 'light'`. Used by WaitlistCTA + EnquiryModal. |
| `components/ui/EnquiryModal.tsx` | Accessible enquiry dialog (focus trap, ESC close, body scroll lock). Source = `'modal'`. |
| `components/onboard/` | Wizard shell + welcome/submitted screens + 9 Step* + `_primitives.tsx` shared UI |
| `components/ui/` | Site-wide primitives (Badge, Button, Card, Container, Eyebrow, SectionHeader, EnquiryModal) |
...
| `lib/constants.ts` | `ADMIN_EMAIL`, `SERVICE_OPTIONS` — single source of truth (frontend side) |
| `lib/types.ts` | `OnboardState`, `OnboardAction`, `BusinessType`, `WaitlistFormData`, `EnquiryFormData`, `EnquirySource` |
| `lib/useEnquirySubmit.ts` | Hook wrapping `submitForm` for the enquiry endpoint (submitting/submitted/error state) |
...
| `supabase/functions/enquiry-submit/` | Deno Edge Function — public, honeypot, validates, inserts enquiry + emails |
...
| Change the hero | [components/sections/Hero.tsx](components/sections/Hero.tsx) + [HeroBrowserMockup.tsx](components/sections/HeroBrowserMockup.tsx). Primary CTA scrolls to `#waitlist`. |
| Edit the waitlist / enquiry form | Fields: [components/forms/EnquiryFormFields.tsx](components/forms/EnquiryFormFields.tsx) (shared). Inline shell: [components/sections/WaitlistCTA.tsx](components/sections/WaitlistCTA.tsx). Modal shell: [components/ui/EnquiryModal.tsx](components/ui/EnquiryModal.tsx). |
| Add a new enquiry field | 3 places: `EnquiryFormData` in [lib/types.ts](lib/types.ts), [EnquiryFormFields.tsx](components/forms/EnquiryFormFields.tsx), and the `enquiries` table + `enquiry-submit` function (if persisted). |
| Add or rename a service option | [lib/constants.ts](lib/constants.ts) `SERVICE_OPTIONS` — single source of truth for the multi-select |
| Add another modal trigger | Local `useState` + `<EnquiryModal open={…} onClose={…} />`. See [Nav.tsx](components/sections/Nav.tsx) or [FAQ.tsx](components/sections/FAQ.tsx) for the pattern. |
...
| Change admin email | **4 places** must stay in sync: `ADMIN_EMAIL` in [lib/constants.ts](lib/constants.ts) (frontend), `ADMIN_NOTIFICATION_EMAIL` Supabase secret (both edge functions read this), `ADMIN_EMAILS` array in [supabase/functions/create-client-user/index.ts](supabase/functions/create-client-user/index.ts) (auth gate), and a new migration to update the `admin_users` seed row |
...
| `NEXT_PUBLIC_ENQUIRY_SUBMIT_URL` | `lib/useEnquirySubmit.ts` (inline form + modal) |
...
| Secret | Used by |
|--------|---------|
| `GMAIL_USER` | all three functions (SMTP from-address) |
| `GMAIL_APP_PASSWORD` | all three functions (SMTP auth) |
| `SITE_URL` | `create-client-user` (invite redirect base) |
| `ADMIN_NOTIFICATION_EMAIL` | `enquiry-submit` + `onboard-submit` (recipient for new-lead notifications). Falls back to `dorza.app@gmail.com`. |
| `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY` | all — auto-injected by Supabase |
...
- ✅ Backend exists. Supabase project provides Postgres + Auth + Storage + Deno Edge Functions.
- ✅ Admin dashboard at `/admin` for reviewing submissions and provisioning clients (now also tabs to Enquiries).
- ✅ Client portal at `/upload` for asset uploads (logo + photos).
- ✅ **Waitlist form is wired (2026-05-18).** Both inline (`WaitlistCTA`) and modal (`EnquiryModal`) surfaces POST to the new `enquiry-submit` Edge Function, which persists to `enquiries` and emails admin + confirmation.
- ✅ **`/onboard` is hidden from the public funnel.** Nav CTA opens `EnquiryModal`; Hero primary CTA scrolls to `#waitlist`; FAQ has a "Talk to us" trigger. `/onboard` remains live (admin shares the URL) and is `noindex`.
- ✅ **Orphaned `/app/waitlist` route deleted.**
